...
export function fetchEvaluationInfo(invite) {
  return new Promise((resolve, reject) => {
    fetch(`${API_URL}/invitation/${invite}/info`, {
      method: "GET",
      headers: {
        // 'Authorization': `Bearer ${localStorage.getItem('token_access')}`,
        // 'Content-type': 'application/json; charset=UTF-8',
...
export function fetchQuestion(invitation) {
  return new Promise((resolve, reject) => {
    fetch(`${API_URL}/invitation/${invitation}`, {
      method: "GET",
      headers: {
        // 'Authorization': `Bearer ${localStorage.getItem('token_access')}`,
        // 'Content-type': 'application/json; charset=UTF-8',
...
export async function sendQuestion(invitation, answer) {
  const params = new URLSearchParams({
    answer: answer,
  });
  return new Promise((resolve, reject) => {
    fetch(`${API_URL}/invitation/${invitation}/?${params}`, {
      method: "POST",
      headers: {
        "Content-type": "application/json; charset=UTF-8",
      },
...
export async function fetchResults(invitation) {
  return new Promise((resolve, reject) => {
    fetch(`${API_URL}/invitation/${invitation}/results`, {
      method: "GET",
      // body: JSON.stringify({
      //     'answer': answer
      // }),
